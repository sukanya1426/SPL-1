...
void keyExpansionCore(unsigned char* in){
    ...
    //S-Box four bytes :
    in[0]= s_box[in[0]];in[1]=s_box[in[1]];//each of the box that we passed in the key expansoin core is  replased by the corresponding value in the s box//
...
void keyExpansion(unsigned char* inputkey){
   ...
   // int bytesGenerated = 16; we have generated 16 bytes so far
    ...
    while(bytesGenerated < 176){
        ...
        for(int i=0;i<4;i++)
        {
           temp[i] = expandedkeys[i+bytesGenerated - 4]; //first - 0-3 second 4-7  third 8-11  
        }
...
void InverseSubByte(unsigned char* state){
      for(int i=0;i<16;i++){
...
void addRoundkey(unsigned char* state,unsigned char* key,int roundNumber){
    for(int i=0;i<16;i++){
        state[i]^=expandedkeys[roundNumber*16 + 1];
    }

}
void InverseAddRoundKey(unsigned char* state,unsigned char* key,int roundNumber){
   for(int i=0;i<16;i++){
       state[i]^=expandedkeys[i+160-roundNumber*16]; 
...
void aes_decrypt(unsigned char* message,unsigned char* state,unsigned char* key){

   InverseAddRoundKey(state,expandedkeys,10);
   
   for(int i=9;i>0;i--){
       InverseShiftRows(state);
       InverseSubByte(state);
       InverseAddRoundKey(state,expandedkeys,i);
       InverseMixColumns(state);
     }
     InverseShiftRows(state);
     InverseSubByte(state);
     InverseAddRoundKey(state,expandedkeys,0);
...
int main()
{
    ...
    for(int i=0;i<lenOfPaddedMessage; i+=16)
         aes_decrypt(paddedMessage+i,state,key);
